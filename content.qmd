...
In this paper, the parametric family of densities is the Gaussian family $\{g_{\mathbf{m}, \boldsymbol{\Sigma}}: \mathbf{m} \in \mathbb{R}^n, \boldsymbol{\Sigma} \in \mathcal{S}^+_n\}$, where $g_{\mathbf{m}, \boldsymbol{\Sigma}}$ denotes the Gaussian density with mean $\mathbf{m} \in \mathbb{R}^n$ and covariance matrix $\boldsymbol{\Sigma} \in \mathcal{S}^+_n$ with $\mathcal{S}^+_n \subset \mathbb{R}^{n \times n}$ the set of symmetric, positive-definite matrices:
	$$
	g_{\mathbf{m},\boldsymbol{\Sigma}}(\mathbf{x})=\dfrac{1}{ (2\pi)^{n/2} \lvert \boldsymbol{\Sigma} \rvert^{1/2}} \exp\left(-\frac{1}{2}(\mathbf{x}-\mathbf{m})^\top\boldsymbol{\Sigma}^{-1}(\mathbf{x}-\mathbf{m})\right), \ \mathbf{x} \in \mathbb{R}^n.
	$$
with $\lvert \boldsymbol{\Sigma} \rvert$ the determinant of $\boldsymbol{\Sigma}$. Moreover, we will consider the Kullback--Leibler (KL) divergence to measure a "distance" between $g^*$ and  $g_{\mathbf{m}, \boldsymbol{\Sigma}}$. Recall that for two densities $f$ and $h$, with $f$ absolutely continuous with respect to $h$, the KL divergence $D(f,h)$ between $f$ and $h$ is defined by: 
	$$
	...
	$$
Thus, our goal is to approximate $g^*$ by $g_{\mathbf{m}^*, \boldsymbol{\Sigma}^*}$ with the optimal mean vector $\mathbf{m}^*$ and the optimal covariance matrix $\boldsymbol{\Sigma}^*$ given by:
	$$
	(\mathbf{m}^*,\boldsymbol{\Sigma}^*) = \arg\min \left\{ D(g^*,g_{\mathbf{m},\boldsymbol{\Sigma}}): \mathbf{m} \in \mathbb{R}^n, \boldsymbol{\Sigma} \in \mathcal{S}_n^+ \right\}.
	$$ {#eq-argminDkl}
This optimization is in general convex and differentiable with respect to $\mathbf{m}$ and $\boldsymbol{\Sigma}$. Moreover, the solution of @eq-argminDkl can be computed analytically by cancelling the gradient. In the Gaussian case, it is thus proved that $\mathbf{m}^*$ and $\boldsymbol{\Sigma}^*$ are simply the mean and variance of the zero-variance density [@RubinsteinKroese_CrossentropyMethodUnified_2011v2], [@RubinsteinKroese_SimulationMonteCarlo_2017v2]:
	$$
	\mathbf{m}^*=\mathbb{E}_{g^*}(\mathbf{X}) \hspace{0.5cm} \text{ and } \hspace{0.5cm} \boldsymbol{\Sigma}^* = \textrm{Var}_{g^*} \left(\mathbf{X}\right).
	$$ {#eq-mstar}
...
As $g^*$ is unknown, the optimal parameters $\mathbf{m}^*$ and $\boldsymbol{\Sigma}^*$ given by @eq-mstar are not directly computable. However, we can sample from the optimal density as it is known up to a multiplicative constant.
Therefore, usual estimation schemes start with estimating $\mathbf{m}^*$ and $\boldsymbol{\Sigma}^*$, say through $\widehat{\mathbf{m}}^*$ and $\widehat{\boldsymbol{\Sigma}}^*$, respectively, and then use these approximations to estimate $\matchcal{E}$ through @eq-hatE with the auxiliary density $g_{\widehat{\mathbf{m}}^*, \widehat{\boldsymbol{\Sigma}}^*}$. Although the estimation of $\matchcal{E}$ with the auxiliary density $g_{\mathbf{m}^*, \boldsymbol{\Sigma}^*}$ usually provides very good results, it is well-known that in high dimensions, the additional error induced by the estimations of $\mathbf{m}^*$ and $\boldsymbol{\Sigma}^*$ severely degrades the accuracy of the final estimation [@PapaioannouEtAl_ImprovedCrossEntropybased_2019], [@UribeEtAl_CrossentropybasedImportanceSampling_2020]. The main problem lies in the estimation of $\boldsymbol{\Sigma}^*$ which, in dimension $n$, involves the estimation of a quadratic (in the dimension) number of terms, namely $n(n+1)/2$.
Recently, the idea to overcome this problem by only evaluating variance terms in a small number of influential directions was explored in [@MasriEtAl_ImprovementCrossentropyMethod_2020] and [@UribeEtAl_CrossentropybasedImportanceSampling_2020]. In these two papers, the auxiliary covariance matrix $\boldsymbol{\Sigma}$ is modeled in the form
	$$
	\boldsymbol{\Sigma} = \sum_{i=1}^k (v_i-1) \mathbf{d}_i \mathbf{d}_i^\top + I_n
	$$ {#eq-Sigmak}
where the $\mathbf{d}_i$'s are the $k$ orthonormal directions which are deemed influential. It is easy to check that $\boldsymbol{\Sigma}$ is the covariance matrix of the Gaussian vector
	$$ v^{1/2}_1 Y_1 \mathbf{d}_1 + \cdots + v^{1/2}_k Y_k \mathbf{d}_k + Y_{k+1} \mathbf{d}_{k+1} + \cdots + Y_n \mathbf{d}_n $$
where the $Y_i$'s are i.i.d. standard normal random variables (one-dimensional), and the $n-k$ vectors $(\mathbf{d}_{k+1}, \ldots, \mathbf{d}_n)$ complete $(\mathbf{d}_1, \ldots, \mathbf{d}_k)$ into an orthonormal basis. In particular, $v_i$ is the variance in the direction of $\mathbf{d}_i$, i.e., $v_i = \mathbf{d}_i^\top \boldsymbol{\Sigma} \mathbf{d}_i$. In @eq-Sigmak, $k$ can be considered as the effective dimension in which variance terms are estimated. In other words, in [@MasriEtAl_ImprovementCrossentropyMethod_2020] and [@UribeEtAl_CrossentropybasedImportanceSampling_2020], the optimal variance parameter is not sought in $\mathcal{S}^+_n$ as in @eq-argminDkl, but rather in the subset of matrices of the form
...
The relevant minimization problem thus becomes
	$$ 
	(\mathbf{m}^*_k, \boldsymbol{\Sigma}^*_k) = \arg\min \left\{ D(g^*,g_{\mathbf{m},\boldsymbol{\Sigma}}): \mathbf{m} \in \mathbb{R}^n, \ \boldsymbol{\Sigma} \in \mathcal{L}_{n,k} \right\}
	$$ {#eq-argminDkl-k}
...
In the rest of the article, we denote as $(\lambda^*_i, \mathbf{d}^*_i)$ the eigenpairs of $\boldsymbol{\Sigma}^*$ ranked in decreasing $\ell$-order and as $({\widehat{\lambda}}^*_i, \widehat{\mathbf{d}}^*_i)$ the eigenpairs of $\widehat{\boldsymbol{\Sigma}}^*$ ranked in decreasing $\ell$-order.
...
The main result of the present paper is to compute the exact value for $\mathbf{\boldsymbol{\Sigma}}^*_k$ in @eq-argminDkl-k, which therefore paves the way for efficient high-dimensional estimation schemes. 

::: {#thm-thm1}
Let $(\lambda^*_i, \mathbf{d}^*_i)$ be the eigenpairs of $\boldsymbol{\Sigma}^*$ ranked in decreasing $\ell$-order. Then for $1 \leq k \leq n$, the solution $(\mathbf{m}^*_k, \boldsymbol{\Sigma}^*_k)$ to @eq-argminDkl-k is given by
$$
\mathbf{m}^*_k = \mathbf{m}^* \ \text{ and } \ \boldsymbol{\Sigma}^*_k = I_n + \sum_{i=1}^k \left( \lambda^*_i - 1 \right) \mathbf{d}^*_i (\mathbf{d}^*_i)^\top. 
$$ {#eq-Sigma-k}
:::

The proof of @thm-thm1 is detailed in [Appendix A](#sec-proof). For $k = 1$ for instance, the matrix $\boldsymbol{\Sigma}^*_1 = I_n + (\lambda_1^*-1) \mathbf{d}_1^* (\mathbf{d}_1^*)^\top$ with $(\lambda_1^*, \mathbf{d}_1^*)$ the eigenpair of $\boldsymbol{\Sigma}^*$ such as $\lambda_1^*$ is either the largest or the smallest eigenvalue of $\boldsymbol{\Sigma}^*$, depending on which one maximizes $\ell$.

This theoretical result therefore suggests to reduce dimension by computing the covariance matrix $\widehat{\boldsymbol{\Sigma}}^*$ and its eigenpairs, rank them in decreasing $\ell$-order and then use the $k$ first eigenpairs $(({\widehat{\lambda}}^*_i, {\widehat{\mathbf{d}}}^*_i), i = 1, \ldots, k)$ to build the covariance matrix $\widehat{\boldsymbol{\Sigma}}^*_k = \sum_{i=1}^k ({\widehat{\lambda}}^*_i-1) {\widehat{\mathbf{d}}}^*_i ({{\widehat{\mathbf{d}}}^*}_i)^\top + I_n$ and the corresponding auxiliary density. This scheme is summarized in Algorithm 1. The effective dimension $k$ is obtained by Algorithm 2, see @sec-choicek below. The proof of the theorem is shown in [Appendix A](#sec-proof).
...
\State - Estimate $\widehat{\mathbf{m}}^*$ and $\widehat{\boldsymbol{\Sigma}}^*$ defined in Equation 8 and Equation 9 with this sample
\State - Compute the eigenpairs $(\widehat{\lambda}^*_i, \widehat{\mathbf{d}}^*_i)$ of $\widehat{\boldsymbol{\Sigma}}^*$ ranked in decreasing $\ell$-order
\State - Compute the matrix $\widehat{\boldsymbol{\Sigma}}^*_k = \sum_{i=1}^k ({\widehat{\lambda}}^*_i-1) {\widehat{\mathbf{d}}}^*_i ({{\widehat{\mathbf{d}}}^*}_i)^\top + I_n$ with $k$ obtained by applying Algorithm 2 with input $({\widehat{\lambda}}^*_1, \ldots, {\widehat{\lambda}}^*_n)$
\State - Generate a new sample $\mathbf{X}_1,\ldots,\mathbf{X}_N$ independently from $g' = g_{\widehat{\mathbf{m}}^*,\widehat{\boldsymbol{\Sigma}}^*_k}$
...
::: {#thm-thm2}
Consider $\boldsymbol{\Sigma} \in \mathcal{L}_{n,1}$ of the form $\boldsymbol{\Sigma} = I_n + (\alpha - 1) \mathbf{d} \mathbf{d}^\top$ with $\alpha > 0$ and $\lVert \mathbf{d} \rVert = 1$. Then the minimizer in $(\alpha, \mathbf{d})$ of the KL divergence between $f$ and $g_{\mathbf{m}^*, \boldsymbol{\Sigma}}$ is $(1+\lVert \mathbf{m}^*\rVert^2, \mathbf{m}^* / \lVert \mathbf{m}^* \rVert)$:
...
In light of these results, it becomes natural to consider the KL divergence with $f$ and not only $g^*$ [@OwenZhou_SafeEffectiveImportance_2000]. Of course, minimizing $D(f, g_{\mathbf{m}, \boldsymbol{\Sigma}})$ without constraints on $\mathbf{m}$ and $\boldsymbol{\Sigma}$ is trivial since $g_{\mathbf{m}, \boldsymbol{\Sigma}} = f$ for $\mathbf{m} = 0$ and $\boldsymbol{\Sigma} = I_n$. However, these choices are the ones we want to avoid in the first place, and so it makes sense to impose some constraints on $\mathbf{m}$ and $\boldsymbol{\Sigma}$. If one keeps in mind the other objective of getting close to $g^*$, then the choice $\mathbf{m} = \mathbf{m}^*$ becomes very natural, and we are led to considering the optimization problem of @thm-thm2 (when $\boldsymbol{\Sigma} \in \mathcal{L}_{n,1}$ is a rank-1 perturbation of the identity).


# Computational framework {#sec-num-results-framework} 

## Numerical procedure for IS estimate comparison
...
1. $\boldsymbol{\Sigma}^*$: the optimal covariance matrix given by @eq-mstar;

2. $\widehat{\boldsymbol{\Sigma}}^*$: the empirical estimation of $\boldsymbol{\Sigma}^*$ given by
$$
	\widehat{\boldsymbol{\Sigma}}^* = \frac{1}{M}\sum_{i=1}^M (\mathbf{X}_i^*-\widehat{\mathbf{m}}^*)(\mathbf{X}_i^*-\widehat{\mathbf{m}}^*)^\top.
$$ {#eq-hatSigma}

The four other covariance matrices considered in the numerical simulations are of the form 
	 $\sum_{i=1}^k (v_i-1) \mathbf{d}_i \mathbf{d}^\top_i + I_n$ where $v_i$ is the variance of $\widehat{\boldsymbol{\Sigma}}^*$ in the direction $\mathbf{d}_i$, $v_i = \mathbf{d}_i^\top \widehat{\boldsymbol{\Sigma}}^* \mathbf{d}_i$. The considered choice of $k$ and $\mathbf{d}_i$ gives the following covariance matrices:   
     
3. ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ is obtained by choosing $\mathbf{d}_i = \mathbf{d}^*_i$ of @thm-thm1, which is supposed to be perfectly known from $\boldsymbol{\Sigma}^*$ and $k$ is computed with Algorithm 2;

4. ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ is obtained by choosing $\mathbf{d}_i = {\widehat{\mathbf{d}}}^*_i$ the $i$-th eigenvector of $\widehat{\boldsymbol{\Sigma}}^*$ (in $\ell$-order), which is an estimation of $\mathbf{d}^*_i$, and $k$ is computed with Algorithm 2;

5. ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ is obtained by choosing $k = 1$ and $\mathbf{d}_1 = \mathbf{m}^* / \lVert \mathbf{m}^* \rVert$;	

6. ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ is obtained by choosing $k = 1$ and $\mathbf{d}_1 = {\widehat{\mathbf{m}}}^* / \lVert {\widehat{\mathbf{m}}}^* \rVert$, where $\widehat{\mathbf{m}}^*$ given by @eq-hatm.  

The matrices ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ use the estimation $\widehat{\boldsymbol{\Sigma}}^*$ with the optimal directions $\mathbf{d}^*_i$ or $\mathbf{m}^*$, while the matrices ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ involve an estimation of these directions from $\widehat{\boldsymbol{\Sigma}}^*$. By definition, $\boldsymbol{\Sigma}^*$ will give optimal results, while results for $\widehat{\boldsymbol{\Sigma}}^*$ will deteriorate as the dimension increases, which is the well-known behavior which we try to improve. Moreover, $\boldsymbol{\Sigma}^*$ and the projection directions $\mathbf{d}^*_i$ or $\mathbf{m}^*$, are of course unknown in practice. For simulation comparison purpose, they could be determined analytically in simple test cases and otherwise we obtained them by a brute force Monte Carlo scheme with a very high simulation budget. Finally, we emphasize that Algorithm 1 corresponds to estimating and projecting on the $\mathbf{d}^*_i$'s, and so the matrix $\widehat{\boldsymbol{\Sigma}}^*_k$ of Algorithm 1 is equal to the matrix ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$. 

|   |$\boldsymbol{\Sigma}^*$|$\widehat{\boldsymbol{\Sigma}}^*$|${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$|${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$|${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$|${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$|
|-------|---|---|---|---|---|---|---|
|Initial covariance matrix|$\boldsymbol{\Sigma}^*$|$\widehat{\boldsymbol{\Sigma}}^*$|$\widehat{\boldsymbol{\Sigma}}^*$|$\widehat{\boldsymbol{\Sigma}}^*$|$\widehat{\boldsymbol{\Sigma}}^*$|$\widehat{\boldsymbol{\Sigma}}^*$|
...
$${#eq-sum}
The quantity of interest $\mathcal{E}$ is defined as $\mathcal{E}=\int_{\mathbb{R}^n} \phi(\mathbf{x}) f(\mathbf{x}) \textrm{d}\mathbf{x} = \mathbb{P}_f(\varphi(\mathbf{X})\geq 0)\simeq 1.35\cdot 10^{-3}$ for all $n$ where the density $f$ is the standard $n$-dimensional Gaussian distribution. Here, the zero-variance density is $g^*(\mathbf{x})=\dfrac{f(\mathbf{x})\mathbb{I}_{\{\varphi(\mathbf{x})\geq 0\}}}{\mathcal{E}}$, and the optimal parameters $\mathbf{m}^*$ and $\boldsymbol{\Sigma}^*$ in @eq-mstar can be computed exactly, namely $\mathbf{m}^* = \alpha \textbf{1}$ with $\alpha = e^{-9/2}/(E(2\pi)^{1/2})$ and $\textbf{1} = \frac{1}{\sqrt n} (1,\ldots,1) \in \mathbb{R}^n$ the normalized constant vector, and $\boldsymbol{\Sigma}^* =(v-1) \mathbf{1} \mathbf{1}^\top + I_n$ with $v=3\alpha-\alpha^2+1$.

###  Evolution of the partial KL divergence and spectrum
@fig-eigsum-1 represents the evolution as the dimension varies between $5$ and $100$ of the partial KL divergence $D'$ for three different choices of covariance matrix: the optimal matrix $\boldsymbol{\Sigma}^*$, its empirical estimation $\widehat{\boldsymbol{\Sigma}}^*$ and the estimation $\widehat{\boldsymbol{\Sigma}}^*_k$ of the optimal lower-dimensional covariance matrix. We can notice that the partial KL divergence for $\widehat{\boldsymbol{\Sigma}}^*$ grows much faster than the other two, and that the partial KL divergence for $\widehat{\boldsymbol{\Sigma}}^*_k$ remains very close to the optimal value $D'(\boldsymbol{\Sigma}^*)$. As the KL divergence is a proxy for the efficiency of the auxiliary density (it is for instance closely related to the number of samples required for a given precision [@Chatterjee18:0]), this suggests that using $\widehat{\boldsymbol{\Sigma}}^*_k$ will provide results close to optimal.

We now check this claim. As $\boldsymbol{\Sigma}^* = (v-1) \textbf{1} \textbf{1}^\top + I_n$, its eigenpairs are $(v, \textbf{1})$ and $(1,\mathbf{d}_i)$ where the $\mathbf{d}_i$'s form an orthonormal basis of the space orthogonal to the space spanned by $\textbf{1}$. In particular, $(v, \textbf{1})$ is the largest (in $\ell$-order) eigenpair of $\boldsymbol{\Sigma}^*$ and $\boldsymbol{\Sigma}^*_k = \boldsymbol{\Sigma}^*$ for any $k \geq 1$.

In practice, we do not use this theoretical knowledge and $\boldsymbol{\Sigma}^*$, $\boldsymbol{\Sigma}^*_k$ and the eigenpairs are estimated. The six covariance matrices introduced in @sec-def_cov and in which we are interested are as follows:

- $\boldsymbol{\Sigma}^* = (v-1) \textbf{1} \textbf{1}^\top + I_n$;
- $\widehat{\boldsymbol{\Sigma}}^*$ given by @eq-hatSigma;
- ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ are equal and given by $(\widehat \lambda-1) \textbf{1} \textbf{1}^\top + I_n$ with $\widehat{\lambda} = \textbf{1}^\top \widehat{\boldsymbol{\Sigma}}^* \textbf{1}$. This amounts to assuming that the projection direction $\textbf{1}$ is perfectly known, whereas the variance in this direction is estimated;
- ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}} = (\widehat{\lambda} - 1) \widehat{\mathbf{d}} {\widehat{\mathbf{d}}}^\top + I_n$ with $(\widehat{\lambda}, \widehat{\mathbf{d}})$ the smallest eigenpair of $\widehat{\boldsymbol{\Sigma}}^*$. The difference with the previous case is that we do not assume anymore that the optimal projection direction $\textbf{1}$ is known, and so it needs to be estimated;
- ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}} = (\widehat{\lambda} - 1) \frac{\widehat{\mathbf{m}}^* {(\widehat{\mathbf{m}}^*)}^\top}{\lVert \widehat{\mathbf{m}}^* \rVert^2} + I_n$ with $\widehat{\mathbf{m}}^*$ given by @eq-hatm and $\widehat{\lambda} = \frac{{(\widehat{\mathbf{m}}^*)}^\top \widehat{\boldsymbol{\Sigma}}^* \widehat{\mathbf{m}}^*}{\lVert \widehat{\mathbf{m}}^* \rVert^2}$. Here we assume that $\mathbf{m}^*$ is a good projection direction, but is unknown and therefore needs to be estimated.
...
@fig-eigsum-2 represents the images by $\ell$ of the eigenvalues of $\boldsymbol{\Sigma}^*$ and $\widehat{\boldsymbol{\Sigma}}^*$. This picture carries a very important insight. We notice that the estimation of most eigenvalues is poor: indeed, all the blue crosses except the leftmost one are meant to be estimator of $1$, whereas we see that they are more or less uniformly spread around $1$. This means that the variance terms in the corresponding directions are poorly estimated, which could be the explanation on why the use of $\widehat{\boldsymbol{\Sigma}}^*$ gives an inaccurate estimation. But what we remark also is that the function $\ell$ is quite flat around one: as a consequence, although the eigenvalues offer significant variability, this variability is smoothed by the action of $\ell$. Indeed, the images of the eigenvalues by $\ell$ take values between $0$ and $0.8$ and have smaller variability. Moreover, $\ell(x)$ increases sharply as $x$ approaches $0$ and thus efficiently distinguishes between the two leftmost estimated eigenvalues and is able to separate them.
...
#| fig-subcap:
#|   - 'Evolution of the partial KL divergence as the dimension increases, with the optimal covariance matrix $\boldsymbol{\Sigma}^*$ (red squares), the sample covariance $\widehat{\boldsymbol{\Sigma}}^*$ (blue circles), and the projected covariance $\widehat{\boldsymbol{\Sigma}}^*_k$ (black dots).'
#|   - 'Computation of $\ell(\lambda_i)$ for the eigenvalues of $\boldsymbol{\Sigma}^*$ (red squares) and $\widehat{\boldsymbol{\Sigma}}^*$ (blue crosses) in dimension $n = 100$.'
#| layout:
#|   - - 45
#|     - -10
#|     - 45
...
plt.plot(range(5,105,5),DKL,'bo',label=r"$D'(\widehat{\boldsymbol{\Sigma}}^*)$")
plt.plot(range(5,105,5),DKLstar,'rs',label=r"$D'(\boldsymbol{\Sigma}^*)$")
plt.plot(range(5,105,5),DKLp,'k.',label=r"$D'(\widehat{\boldsymbol{\Sigma}}^*_k)$")
...
plt.plot(Table_eigv[:,0],Table_eigv[:,1],'bx',\
        label=r"Eigenvalues of $\widehat{\boldsymbol{\Sigma}}^*$")
plt.plot(Table_eigv_st[:,0],Table_eigv_st[:,1],'rs',\
         label=r"Eigenvalues of $\boldsymbol{\Sigma}^*$")
plt.legend(fontsize=16)
plt.show()
```

###  Numerical results
We report in @tbl-sum the numerical results for the six different matrices and the vMFN model for the dimension $n=100$. The column $\boldsymbol{\Sigma}^*$ gives the optimal results, while the column $\widehat{\boldsymbol{\Sigma}}^*$ corresponds to the results that we are trying to improve. Comparing these two columns, we notice as expected that the estimation of $\mathcal{E}$ with $\widehat{\boldsymbol{\Sigma}}^*$ is significantly degraded. Compared to the first column $\boldsymbol{\Sigma}^*$, the third and fourth columns with ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}} =  {\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ correspond to the best projection direction $\textbf{1}$ (as for $\boldsymbol{\Sigma}^*$) but estimating the variance in this direction (instead of the true variance) with $\textbf{1}^\top \widehat{\boldsymbol{\Sigma}}^* \textbf{1}$. This choice performs very well, with numerical results similar to the optimal ones. This can be understood since in this case, both ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and $\boldsymbol{\Sigma}^*$ are of the form $\alpha \textbf{1} \textbf{1}^\top + I_n$ and so estimating ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ requires only a one-dimensional estimation (namely, the estimation of $\alpha$). Next, the last two columns ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ highlight the impact of having to estimate the projection directions in addition to the variance since these two matrices are of the form $\widehat \alpha \widehat{\textbf{1}} {\widehat{\textbf{1}}}^\top + I_n$ with both $\widehat{\alpha}$ (the variance term) and $\widehat{\textbf{1}}$ (the direction) being estimated. We observe that these matrices yield results which are close to optimal and greatly improve the estimation obtained using $\widehat{\boldsymbol{\Sigma}}^*$. In dimension $100$, the coefficient of variation is around $5.1$\% for ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$, and around $6.2$\% for $\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_{\text{opt}}$, compared to $2.6$\% for $\widehat{\boldsymbol{\Sigma}}^*$. 

Moreover, we observe that ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ gives better results than ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$. We suggest that this is because $\widehat{\mathbf{m}}^* / \lVert \widehat{\mathbf{m}}^* \rVert$ is a better estimator of $\textbf{1}$ than the eigenvector of $\widehat{\boldsymbol{\Sigma}}^*$. Indeed, evaluating $\widehat{\mathbf{m}}^*$ requires the estimation of $n$ parameters, whereas $\widehat{\boldsymbol{\Sigma}}^*$ needs around $n^2/2$ parameters to estimate, so the eigenvector is finally more noisy than the mean vector.  In the last column, we present the vMFN estimation that is slightly more efficicent than the estimation obtained with ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$.
...
Markdown(tabulate(
  table, 
  headers=["","$\boldsymbol{\Sigma}^*$", "$\widehat{\boldsymbol{\Sigma}}^*$", "$\widehat{\boldsymbol{\Sigma}}_{opt}$",\
           "$\widehat{\boldsymbol{\Sigma}}_{mean}$", "${\widehat{\boldsymbol{\Sigma}}^{+d}_{opt}}$",\
           "$\widehat{\boldsymbol{\Sigma}}^{+d}_{mean}$", "vMFN"],
    tablefmt="pipe"))
```
...
$$ {#eq-parabol}
		The quantity of interest $\mathcal{E}$ is defined as $\mathcal{E}=\int_{\mathbb{R}^n} \phi(\mathbf{x}) f(\mathbf{x}) \textrm{d}\mathbf{x} = \mathbb{P}_f(\varphi(\mathbf{X})\geq 0)$ for all $n$ where the density $f$ is the standard $n$-dimensional Gaussian distribution. This function is motivated in part because $\mathbf{m}^*$ and $\mathbf{d}^*_1$ are different and also because Algorithm 2 chooses two projection directions. Thus, this is an example where ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ are significantly different.

###  Evolution of the partial KL divergence and spectrum
We check on @fig-inefficiency-parab-1 that the partial KL divergence obeys the same behavior as for the previous example, namely the one associated with $\widehat{\boldsymbol{\Sigma}}^*$ increases much faster than the ones associated with $\boldsymbol{\Sigma}^*$ and $\widehat{\boldsymbol{\Sigma}}^*_k$, which again suggests that projecting can improve the situation. Since the function $\varphi$ only depends on the first three variables and is even in $x_2$ and $x_3$, one gets that $\mathbf{m}^* = \alpha 
	\textbf{e}_1$ with $\alpha = \mathbb{E}(X_1 \mid X_1 \geq 25 X^2_2 + 30 X^2_3 + 1) \approx 1.9$ (here and in the sequel, $\textbf{e}_i$ denotes the $i$th canonical vector of $\mathbb{R}^n$, i.e., all its coordinates are $0$ except the $i$-th one which is equal to one), and that $\boldsymbol{\Sigma}^*$ is diagonal with
$$ \boldsymbol{\Sigma}^* =
	\begin{pmatrix}
	...
	\end{pmatrix}. 
 $$
Note that the off-diagonal elements of the submatrix $(\boldsymbol{\Sigma}^*_{ij})_{1 \leq i, j \leq 3}$ are indeed $0$ since they result from integrating an odd function of an odd random variable with an even conditioning. For instance, if $F(x) = \mathbb{P}(30 X^2_3 + 1 \leq x)$, then by conditioning on $(X_1, X_3)$ we obtain
$$
	\boldsymbol{\Sigma}^*_{12} = \mathbb{E} \left( (X_1 - \alpha) X_2 \mid X_1 - 25 X_2^2 \geq 30 X^2_3 + 1 \right)\\
...
###  Numerical results
The numerical results of our simulations are presented in @tbl-parabol. We remark as before that, when using $\widehat{\boldsymbol{\Sigma}}^*$, the accuracy quickly deteriorates as the dimension increases as shows the coefficient of variation of $396 \%$ in dimension $n = 100$. In contrast, ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ leads to very accurate results, which remain close to optimal up to the same dimension $n = 100$. This behavior is to compare with the evolution of the relative KL divergence: contrary to $\widehat{\boldsymbol{\Sigma}}^*$, ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ gives a partial KL divergence close to optimal in dimension $n = 100$. This confirms that the KL divergence is indeed a good proxy to assess the relevance of an auxiliary density.

It is also interesting to note that the direction $\mathbf{m}^*$ improves the situation compared to not projecting (column ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ compared to $\widehat{\boldsymbol{\Sigma}}^*$), but using ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ gives significantly better results, with for instance a coefficient of variation around $3.6 \%$ for ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and around $28.8 \%$ for ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ in dimension $n = 100$. Thus, this confirms our theoretical result that the $\mathbf{d}^*_i$'s are good directions on which to project. 
	
Finally, we notice that performing estimations of the projection directions instead of taking the true ones (columns ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ vs ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$) slightly degrades the situation, making the coefficient of variation increase from $3.6$ to $9.5 \%$ even if the accuracy remains satisfactory.
	The vMFN model is also not really adapted to this example as it gives results similar to ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$. Gaussian density family are more able to fit $g^*$ than vMFN parametric model in this test case.
...
$$ {#eq-banana}
 The term $g_{{\bf 0},C}$ represents the pdf of a Gaussian distribution of mean ${\bf 0}$ and diagaonal covariance matrix $C=\text{diag}(\sigma^2,1,\dots,1)$. The value of $b$ and $\sigma^2$ are respectively set to $b=800$ and $\sigma^2=0.0025$. We choose $\phi$ such that the optimal IS density $g^*$ is equal to $h$, i.e., we choose $\phi = h/f$ so that the integral $\mathcal{E}$ that we are trying to estimate is equal to $\mathcal{E} = \int \phi f = 1$. This choice is made in order to have an optimal covariance matrix $\boldsymbol{\Sigma}^*$ whose two largest eigenvalues (in $\ell$-order) correspond to the smallest and largest eigenvalues, as can be seen in @fig-inefficiency-banana-2. More formally, the optimal value of the Gaussian parameters are given by $\mathbf{m}^*={\bf 0}$ and $\boldsymbol{\Sigma}^*$ is diagonal with
$$ \boldsymbol{\Sigma}^* =
	\begin{pmatrix}
	...
	\end{pmatrix}.
$$
The evolution of the KL partial divergence is given in @fig-inefficiency-banana-1. As the optimal mean $\mathbf{m}^*$ is equal to ${\bf 0}$, we cannot project on $\mathbf{m}^*$ and so the matrix ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ is not defined. However, the numerical estimation $\widehat{\mathbf{m}}^*$ will not be equal to $0$ and so the approach proposed in [@MasriEtAl_ImprovementCrossentropyMethod_2020] with ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ is still applicable numerically.

The simulation results for the different covariance matrices and the vMFN density are given in @tbl-banana. The matrices ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ perform very well for the estimation of $E$ with an accuracy of the same order as the optimal covariance matrix $\boldsymbol{\Sigma}^*$. The effect of estimating the $k=2$ main projection directions does not affect much the estimation performance as ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ is still efficient compared to ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$. The estimation results with ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ are not really accurate and this choice is in fact roughly equivalent to choosing a random projection direction. The vMFN parametric model is not adapted to this test case as the vMFN estimate is not close to 1.

```{python}
#| label: fig-inefficiency-banana
#| fig-cap: Partial KL divergence and spectrum for the banana shape example.
#| fig-subcap:
#|   - 'Evolution of the partial KL divergence as the dimension increases, with the optimal covariance matrix $\boldsymbol{\Sigma}^*$ (red saquares), the sample covariance $\widehat{\boldsymbol{\Sigma}}^*$ (blue circles), and the projected covariance $\widehat{\boldsymbol{\Sigma}}^*_k$ (black dots).'
#|   - 'Computation of $\ell(\lambda_i)$ for the eigenvalues of $\boldsymbol{\Sigma}^*$ (red squares) and $\widehat{\boldsymbol{\Sigma}}^*$ (blue crosses) in dimension $n = 100$ for the banana shape example of @eq-banana.'
#| layout:
#|   - - 45
#|     - -10
#|     - 45
...
#### plot of partial KL divergence
plt.plot(range(5,n+1,5),DKL,'bo',label=r"$D'(\widehat{\boldsymbol{\Sigma}}^*)$")
plt.plot(range(5,n+1,5),DKLstar,'rs',label=r"$D'(\boldsymbol{\Sigma}^*)$")
plt.plot(range(5,n+1,5),DKLp,'k.',label=r"$D'(\widehat{\boldsymbol{\Sigma}}^*_k)$")
...
The reference value of this probability $\mathcal{E}$ is reported in @tbl-portfolio for dimension $n=100$. The optimal parameters $\mathbf{m}^*$ and $\boldsymbol{\Sigma}^*$ cannot be computed analytically, but they are accurately estimated by Monte Carlo with a large sample. It turns out that $\mathbf{m}^*$ and the first eigenvector $\mathbf{d}^*_1$ of $\boldsymbol{\Sigma}^*$ are numerically indistinguishable and that Algorithm 2 selects $k=1$ projection direction, so that numerically, the choices ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ are indistinguishable and gives the same estimation results. Actually, the fact that these two estimators behave similarly does not seem to come from the fact that $\mathbf{m}^*$ and $\mathbf{d}^*$ are close: this relation can be broken for instance by a simple translation argument (see remark after @tbl-payoff), but even then they behave similarly. The KL partial divergence and the spectrum with the associated $\ell$-order are presented respectively in @fig-inefficiency-portfolio-1 and in  @fig-inefficiency-portfolio-2.
...
#| fig-subcap:
#|   - 'Evolution of the partial KL divergence as the dimension increases, with the optimal covariance matrix $\boldsymbol{\Sigma}^*$ (red squares), the sample covariance $\widehat{\boldsymbol{\Sigma}}^*$ (blue circles), and the projected covariance $\widehat{\boldsymbol{\Sigma}}^*_k$ (black dots).'
#|   - 'Computation of $\ell(\lambda_i)$ for the eigenvalues of $\boldsymbol{\Sigma}^*$ (red squares) and $\widehat{\boldsymbol{\Sigma}}^*$ (blue crosses) in dimension $n = 100$  for the large portfolio losses of @eq-portfolio.'
#| layout:
#|   - - 45
#|     - -10
#|     - 45
...
The results of @tbl-portfolio show similar trends as for the first test case of @sec-sub:sum. First, projecting seems indeed a relevant idea, as using ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ or ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ greatly improves the situation compared to $\widehat{\boldsymbol{\Sigma}}^*$. This is particularly salient as $\widehat{\boldsymbol{\Sigma}}^*$ yields an important bias and a coefficient of variation of $370\%$, whereas projecting on $\mathbf{d}^*_1$ or $\mathbf{m}^*$ yields a coefficient of variation of $12 \%$ and of $7.5 \%$ respectively. This improvement is still true even when the projection directions are estimated: compared to ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$, ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ give coefficients of variation between $12.2$ and $14.9\%$. Finally, ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ seems to behave better than ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$. 
...
$$ {#eq-payoff}
	where $[y]_+=\max(y,0)$, for a real number $y$. The constants are taken from [@Kawai_OptimizingAdaptiveImportance_2018]: $S_0=50$, $r=0.05, T=0.5, \sigma=0.1, K=55$, where they test the function for dimension $n=16$. In our contribution, we test this example in dimension $100$. Concerning $\mathbf{m}^*$ and the $\mathbf{d}^*_i$'s, the situation is the same as in the previous example: they are not available analytically but can be estimated numerically by Monte Carlo with a large simulation budget. And again, it turns out that $\mathbf{m}^*$ and the first eigenvector $\mathbf{d}^*_1$ of $\boldsymbol{\Sigma}^*$ are numerically indistinguishable and that Algorithm 2 selects $k=1$ projection direction, so that ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ yield results that are numerically indistinguishable. The KL partial divergence and the spectrum with the associated $\ell$-order are respectively presented in @fig-inefficiency-kawai-1 and @fig-inefficiency-kawai-2. 

The results of this example are given in @tbl-payoff. The insight gained in the previous examples is confirmed. Projecting on $\mathbf{m}^*$ or $\mathbf{d}^*_1$ in dimension $n = 100$ enables to reach convergence and reduces (compared to $\widehat{\boldsymbol{\Sigma}}^*$) the coefficient of variation from $559\%$ to nearly $2\%$. Moreover, this improvement goes through even when projection directions are estimated, with again ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ behaving better than ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$.
...
#| fig-subcap:
#|   - 'Evolution of the partial KL divergence as the dimension increases, with the optimal covariance matrix $\boldsymbol{\Sigma}^*$ (red squares), the sample covariance $\widehat{\boldsymbol{\Sigma}}^*$ (blue circles), and the projected covariance $\widehat{\boldsymbol{\Sigma}}^*_k$ (black dots).'
#|   - 'Computation of $\ell(\lambda_i)$ for the eigenvalues of $\boldsymbol{\Sigma}^*$ (red squares) and $\widehat{\boldsymbol{\Sigma}}^*$ (blue crosses) in dimension $n = 100$ for the Asian payoff example of @eq-payoff'
#| layout:
#|   - - 45
#|     - -10
#|     - 45
...
::: {.remark}
As already mentioned, the two directions $\mathbf{m}^*$ and $\mathbf{d}^*_1$ are numerically indistinguishable in the two examples of @sec-sub:portfolio and @sec-sub:payoff. However, we do not believe this relation to be highly relevant. For instance, this symmetry can be broken by changing $\phi$ into $\phi' = \phi(\cdot - \mu)$ and $f$ into $f' = f(\cdot - \mu)$ for some $\mu \in \mathbb{R}^n$. Since $g^* \propto \phi f$, this amounts to translating $g^*$ which thus changes $\mathbf{m}^*$ into ${\mathbf{m}^*}' = \mathbf{m}^* + \mu$, but which does not change the covariance matrix (and therefore its leading eigenvector $\mathbf{d}^*_1$) which is translation-invariant. Note that this translation does not affect the integral $\mathcal{E} = \int \phi' f' = \int \phi f$, and so this modification leads to a new estimator $\widehat{\mathcal{E}}_\mu$ of the same quantity $\mathcal{E}$. However, it can be shown that $\widehat{\mathcal{E}}_\mu$ and $\widehat{\mathcal{E}}$ (the estimators considered throughout the paper) have the same law so that this translation, although it does break the relation $\mathbf{m}^* \approx \mathbf{d}^*_1$, does not change the law of the estimators. This suggests that, if the estimators based on ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ do behave similarly on these examples, this is not due to the fact that $\mathbf{m}^*$ and $\mathbf{d}^*_1$ are close but rather to @thm-thm1 and @thm-thm2. However, the fact that $\mathbf{m}^*$ and $\mathbf{d}^*_1$ are close bears some insight into the importance of the quality of the estimation of the projection direction as we now elaborate in the conclusion.
:::

# Conclusion {#sec-Ccl}
	
The goal of this paper is to assess the efficiency of projection methods in order to overcome the curse of dimensionality for importance sampling. Based on a new theoretical result (@thm-thm1), we propose to project on the subspace spanned by the eigenvectors $\mathbf{d}^*_i$'s corresponding to the largest eigenvalues of the optimal covariance matrix $\boldsymbol{\Sigma}^*$, where eigenvalues are ranked based on their image by some explicit function $\ell$. Our numerical results show that if the $\mathbf{d}^*_i$'s were perfectly known, then projecting on them (column ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ in the result tables of @sec-test-cases) would greatly improve the final estimation compared to using the empirical estimation of the covariance matrix (column $\widehat{\boldsymbol{\Sigma}}^*$) and actually lead to results which are comparable to those obtained with the optimal covariance matrix (column $\boldsymbol{\Sigma}^*$). Moreover, we show that this improvement goes through when one estimates the $\mathbf{d}^*_i$'s (column ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$) by computing the eigenpairs of $\widehat{\boldsymbol{\Sigma}}^*$: indeed, using ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ as covariance matrix instead of $\widehat{\boldsymbol{\Sigma}}^*$ gives results which remain accurate up to the dimension $n = 100$ considered in the present paper.
	
Moreover, we compare the directions $\mathbf{d}^*_i$'s, which are justified by @thm-thm1, with the algorithm proposed in [@MasriEtAl_ImprovementCrossentropyMethod_2020] which amounts to projecting on $\mathbf{m}^*$ (column ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ when $\mathbf{m}^*$ is assumed to be known, or column ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ when one uses the estimation $\widehat{\mathbf{m}}^*$ of $\mathbf{m}^*$). On three out of the five numerical examples considered, it turns out that $\mathbf{m}^*$ and $\mathbf{d}^*_1$ can be proved to be equal, or are numerically indistinguishable, and Algorithm 2 selects one projection direction. In these cases, the choices ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{opt}}$ and ${\widehat{\boldsymbol{\Sigma}}^{\text{}}_\text{mean}}$ lead to similar numerical results. The second and third test cases of @sec-sub:parabol break the relation between $\mathbf{m}^*$ and $\mathbf{d}^*_1$, and in this case, ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ clearly outperforms ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$. It would be interesting to delve deeper into the relation between $\mathbf{m}^*$ and the eigenvectors of $\boldsymbol{\Sigma}^*$, and try and understand when estimating the $\mathbf{d}^*_i$'s instead of the simpler $\mathbf{m}^*$ is indeed worthwhile.
...
For the first point, remember that we made the choice to estimate the eigenpairs of $\boldsymbol{\Sigma}^*$ by computing the eigenpairs of $\widehat{\boldsymbol{\Sigma}}^*$. Moreover, in the numerical examples of @sec-sub:sum, @sec-sub:portfolio and @sec-sub:payoff where $\mathbf{m}^*$ and $\mathbf{d}^*_1$ are equal or indistinguishable, we saw that ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{mean}}$ performed better than ${\widehat{\boldsymbol{\Sigma}}^{\text{+d}}_\text{opt}}$ and we conjecture that this is because $\widehat{\mathbf{m}}$ is a better estimator than $\widehat{\mathbf{d}}^*_1$ (recall that $\mathbf{m}^* = \mathbf{d}^*_1$ for the example of @sec-sub:sum, while in @sec-sub:portfolio and @sec-sub:payoff they are numerically indistinguishable and so, for all practical purposes, $\widehat{\mathbf{m}}^*$ and $\widehat{\mathbf{d}}^*_1$ can be considered estimators of the same direction). This suggests that improving the estimation of the $\mathbf{d}^*_i$'s can indeed improve the final estimation of $\mathcal{E}$. Possible ways to do so consist in adapting existing results on the estimation of covariance matrices (for instance [@LedoitWolf_WellconditionedEstimatorLargedimensional_2004]) or even directly results on the estimation of eigenvalues of covariance matrices such as [@Benaych-Georges11:0], [@Mestre_ImprovedEstimationEigenvalues_2008], [@Mestre_AsymptoticBehaviorSample_2008], [@Nadakuditi08:0], which we plan to do in future work. Moreover, it would be interesting to relax the assumption that one can sample from $g^*$ in order to estimate $\widehat{\boldsymbol{\Sigma}}^*$. For the second point, we plan to investigate how the idea of the present paper can improve the efficiency of adaptive importance sampling schemes in high dimensions. In this case, there is an additional difficulty, namely the introduction of likelihood ratios can lead to the problem of weight degeneracy which is another reason why performance of such schemes degrades in high dimensions [@BengtssonEtAl_CurseofdimensionalityRevisitedCollapse_2008}].
...
::: {#lem-D}
Let $f$ be the density of the standard Gaussian vector in dimension $n$, $\phi: \mathbb{R}^n \to \mathbb{R}_+$ and $g_* = f \phi / \mathcal{E}$ with $\mathcal{E} = \int f \phi$. Then for any $\mathbf{m}$ and any $\boldsymbol{\Sigma}$ of the form $\boldsymbol{\Sigma} = I_n + \sum_i (\alpha_i - 1) \mathbf{d}_i \mathbf{d}_i^\top$ with $\alpha_i > 0$ and the $\mathbf{d}_i$'s orthonormal, we have
$$
\begin{aligned}
 D(g^*, g_{\mathbf{m}, \boldsymbol{\Sigma}}) =&  \frac{1}{2} \sum_i \left( \log \alpha_i - \left(1 - \frac{1}{\alpha_i} \right) \mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i \right) + \frac{1}{2} (\mathbf{m} - \mathbf{m}^*)^\top \boldsymbol{\Sigma}^{-1} (\mathbf{m} - \mathbf{m}^*)\\
...
For any $\mathbf{m} \in \mathbb{R}^n$ and $\boldsymbol{\Sigma} \in \mathcal{S}^+_n$, we have by definition
$$ D(g^*, g_{\mathbf{m}, \boldsymbol{\Sigma}}) = \mathbb{E}_{g^*} \left( \log \left( \frac{g^*(\mathbf{X})}{g_{\mathbf{m}, \boldsymbol{\Sigma}}(\mathbf{X})} \right) \right) = \mathbb{E}_{g^*} \left( \log \left( \frac{\frac{\phi(\mathbf{X}) e^{-\frac{1}{2} \lVert \mathbf{X} \rVert^2}}{\mathcal{E}(2\pi)^{n/2}}}{ \frac{e^{-\frac{1}{2} (\mathbf{X} - \mathbf{m})^\top \boldsymbol{\Sigma}^{-1} (\mathbf{X} - \mathbf{m})}}{(2\pi)^{n/2} \lvert \boldsymbol{\Sigma} \rvert^{1/2}} } \right) \right) $$
and so
$$
\begin{aligned}
D(g^*, g_{\mathbf{m}, \boldsymbol{\Sigma}}) = &- \frac{1}{2} \mathbb{E}_{g^*}(\lVert \mathbf{X} \rVert^2) + \frac{1}{2} \mathbb{E}_{g^*} \left( (\mathbf{X} - \mathbf{m})^\top \boldsymbol{\Sigma}^{-1} (\mathbf{X} - \mathbf{m}) \right)\\
& + \frac{1}{2} \log \lvert \boldsymbol{\Sigma} \rvert - \log \mathcal{E} + \mathbb{E}_{g^*}(\log \phi(\mathbf{X})).
\end{aligned}
$$
...
$$
\mathbb{E}_{g^*} \left( (\mathbf{X} - \mathbf{m})^\top \boldsymbol{\Sigma}^{-1} (\mathbf{X} - \mathbf{m}) \right) = \mathbb{E}_{g^*} \left( (\mathbf{X} - \mathbf{m}^*)^\top \boldsymbol{\Sigma}^{-1} (\mathbf{X} - \mathbf{m}^*) \right)\\
+ (\mathbf{m} - \mathbf{m}^*)^\top \boldsymbol{\Sigma}^{-1} (\mathbf{m} - \mathbf{m}^*).
$$
...
$$
\mathbb{E}_{g^*}\left[ \lVert \mathbf{X} - \mathbf{m}^* \rVert^2 \right] = \mathbb{E}_{g^*} \left[ \textrm{tr}((\mathbf{X} - \mathbf{m}^*) (\mathbf{X} - \mathbf{m}^*)^\top) \right] = \textrm{tr} (\boldsymbol{\Sigma}^*) 
$$
and we obtain with similar arguments $\mathbb{E}_{g^*}( (\mathbf{X} - \mathbf{m}^*)^\top \boldsymbol{\Sigma}^{-1} (\mathbf{X} - \mathbf{m}^*) ) = \textrm{tr} ( \boldsymbol{\Sigma}^{-1} \boldsymbol{\Sigma}^*)$. Consider now $\boldsymbol{\Sigma} = I_n + \sum_i (\alpha_i - 1) \mathbf{d}_i \mathbf{d}_i^\top$ with $\alpha_i > 0$ and the $\mathbf{d}_i$'s orthonormal. Then the eigenvalues of $\boldsymbol{\Sigma}$ potentially different from $1$ are the $\alpha_i$'s ($\alpha_i$ is the eigenvalue associated with $\mathbf{d}_i$), so that
$$\log \lvert \boldsymbol{\Sigma} \rvert = \sum_i \log \alpha_i. 
$$
Moreover, we have $\boldsymbol{\Sigma}^{-1} = I_n - \sum_i \beta_i \mathbf{d}_i \mathbf{d}_i^\top$ with $\beta_i = 1 - 1/\alpha_i$ and so
$$
\textrm{tr}(\boldsymbol{\Sigma}^{-1} \boldsymbol{\Sigma}^*) = \textrm{tr}(\boldsymbol{\Sigma}^*) - \sum_i \beta_i \mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i. 
$$
...
From @eq-D we see that the only dependency of $D(g^*, g_{\mathbf{m}, \boldsymbol{\Sigma}})$ in $\mathbf{m}$ is in the quadratic term $(\mathbf{m} - \mathbf{m}^*)^\top \boldsymbol{\Sigma}^{-1} (\mathbf{m} - \mathbf{m}^*)$. As $\boldsymbol{\Sigma}$ is definite positive, this term is $\geq 0$, and so it is minimized for $\mathbf{m} = \mathbf{m}^*$. Next, we see that the derivative in $\alpha_i$ is given by (here and in the sequel, we see $D(g^*, g_{\mathbf{m}, \boldsymbol{\Sigma}})$ as a function of $\mathbf{v} = (\alpha_i)_i$ and $\mathbf{d} = (\mathbf{d}_i)_i$)
	$$ \dfrac{\partial D}{\partial \alpha_i}(\mathbf{v}, \mathbf{d}) = \dfrac{1}{\alpha_i} - \frac{1}{\alpha_i^2} \mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i = \frac{1}{\alpha_i^2} \left( \alpha_i - \mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i \right). $$
Thus, for fixed $\mathbf{d}$, $D$ is decreasing in $\alpha_i$ for $\alpha_i < \mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i$ and then increasing for $\alpha_i > \mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i$, which shows that, for fixed $\mathbf{d}$, it is minimized for $\alpha_i = \mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i$. For this value (and $\mathbf{m} = \mathbf{m}^*$) we have
$$ 
D(g^*, g_{\mathbf{m}^*, \boldsymbol{\Sigma}}) = \sum_{i=1}^k \left[ \log(\mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i) + 1 - \mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i \right] + C = -\sum_{i=1}^k \ell(\mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i) + C
$$ {#eq-Dell}
with $C = - \frac{1}{2} \lVert \mathbf{m}^* \rVert^2 - \log \mathcal{E} + \mathbb{E}_{g^*}(\log \phi(\mathbf{X}))$ independent from the $\mathbf{d}_i$'s. Since $\ell$ is decreasing and then increasing, it is clear from this expression that in order to minimize $D$, one must choose the $\mathbf{d}_i$'s in order to either maximize or minimize $\mathbf{d}_i^\top \boldsymbol{\Sigma}^* \mathbf{d}_i$, whichever maximizes $\ell$. Since the variational characterization of eigenvalues shows that eigenvectors precisely solve this problem, we get the desired result.
:::
...
In @eq-D, the $\mathbf{m}^*$ and the $\boldsymbol{\Sigma}^*$ that appear in the right-hand side are the mean and variance of the density $g^*$ considered in the first argument of the Kullback--Leibler divergence. In particular, if we apply @eq-D with $\phi \equiv 1$, we have $g^* = f$, and the $\mathbf{m}^*$ and $\boldsymbol{\Sigma}^*$ of the right-hand side become $0$ and $I_n$, respectively, so that
$$ 
 D(f, g_{\mathbf{m}, \boldsymbol{\Sigma}}) =  \frac{1}{2} \sum_i \left( \log \alpha_i - \left(1 - \frac{1}{\alpha_i} \right) \right) + \frac{1}{2} \mathbf{m}^\top \boldsymbol{\Sigma}^{-1} \mathbf{m}. 
$$
Now, if we consider $\mathbf{m} = \mathbf{m}^*$ and $\boldsymbol{\Sigma} = I + (\alpha - 1) \mathbf{d} \mathbf{d}^\top$, we obtain (using $\boldsymbol{\Sigma}^{-1} = I - (1-1/\alpha) \mathbf{d} \mathbf{d}^\top$ as mentioned in the proof of @lem-D)
$$
D(f, g_{\mathbf{m}^*, \boldsymbol{\Sigma}}) =  \frac{1}{2} \left( \log \alpha - \left(1 - \frac{1}{\alpha} \right) \left( 1 + (\mathbf{d}^\top \mathbf{m}^*)^2 \right) \right) + \frac{1}{2} \lVert \mathbf{m}^* \rVert^2.
$$
Then the function $x \mapsto \log x + (1/x-1)\gamma$ is minimized for $x = \gamma$ where it takes the value $-\ell(\gamma)$: $D(f, g_{\mathbf{m}^*, \boldsymbol{\Sigma}})$ is therefore minimized for $\alpha = 1 + (\mathbf{d}^\top \mathbf{m}^*)^2$ and for this value, we have
$$
D(f, g_{\mathbf{m}^*, \boldsymbol{\Sigma}}) =  - \frac{1}{2} \ell(1 + (\mathbf{d}^\top \mathbf{m}^*)^2) + \frac{1}{2} \lVert \mathbf{m}^* \rVert^2. 
$$
